    ...
    <header className="border-b border-gold/20 bg-night">
      <div className="mx-auto flex max-w-5xl items-center justify-between gap-3 px-4 py-4 sm:px-6">
        <Link href="/" className="leading-tight">
          ...
          <span className="block text-[9px] tracking-[0.35em] text-paper">FIND · INVEST · GROW</span>
        </Link>

        <nav className="hidden items-center gap-x-5 text-sm md:flex">
          {nav.map((item) => (
            <Link key={item.href} href={item.href} className="text-paper transition-colors hover:text-gold-bright">{item.label}</Link>
          ))}
          ...
          <MobileNav
            ...
            footer={
              <div>
                ...
                <Link href="/admin" className="mt-4 block text-center text-xs tracking-[0.2em] text-mist hover:text-gold-bright">AGENT PORTAL</Link>
              </div>
            }
          />
        </div>
