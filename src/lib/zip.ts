...
export async function buildClientZip(
  ...
  if (totalBytes > MAX_ZIP_BYTES) {
    return {
      ok: false,
      status: 413,
      error: "Too large for a single ZIP. Download documents individually.",
    };
  }

  const admin = createAdminClient();
  const zip = new JSZip();
  const root = folderSafe(clientName);
  ...
  for (const doc of docs) {
    ...
    let n = 2;
    while (usedNames.has(`${catFolder}/${entryName}`)) {
    ...
    usedNames.add(`${catFolder}/${entryName}`);
