        ...
        <div className="mt-12 grid gap-10 md:grid-cols-2">
          <div>
            ...
            <div className="mt-8 space-y-3 text-sm">
              ...
              <p><a href={`tel:${settings.phone.replace(/[^+\d]/g, "")}`} className="text-paper transition-colors hover:text-paper">Call — {settings.phone}</a></p>
              <p><a href={`mailto:${settings.email}`} className="text-paper transition-colors hover:text-paper">Email — {settings.email}</a></p>
            </div>
            <p className="mt-8 text-xs text-paper">
          ...
          <div>
            {sent ? (
              <div className="rounded-lg border border-gold/30 bg-night-soft px-6 py-8 text-center">
                ...
                <p className="mt-2 text-sm text-paper">Thank you — I will be in touch soon.</p>
              </div>
            ) : (
              <form action={submitEnquiry} className="space-y-4">
                ...
                {error && (
                  <p className="rounded-md border border-hibiscus/40 bg-hibiscus/10 px-3.5 py-2.5 text-sm text-paper">
