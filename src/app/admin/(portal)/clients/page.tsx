      ...
      {rows.length === 0 ? (
        ...
        <>
              ...
              <tbody>
                {rows.map((c) => (
                  <tr key={c.id} className="border-b border-sand/60 last:border-0">
                    ...
                    <td className="px-4 py-3">
                      <Link href={`/admin/clients/${c.id}`} className="font-medium text-ink hover:text-gold-deep">{c.full_name}</Link>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            <button type="submit" form="export-form" name="scope" value="selected" className={exportBtn}>Download Selected (.zip)</button>
            <button type="submit" form="export-form" name="scope" value="all" className={exportBtn}>Download All Clients (.zip)</button>
          </div>
          ...
          <div className="mt-6 flex items-center justify-between text-sm text-stone">
            ...
            <div className="flex gap-3">
              {pageNum > 1 && (
                <Link href={`/admin/clients?q=${encodeURIComponent(search)}&page=${pageNum - 1}`} className="hover:text-ink">← Previous</Link>
              )}
              {pageNum < totalPages && (
                <Link href={`/admin/clients?q=${encodeURIComponent(search)}&page=${pageNum + 1}`} className="hover:text-ink">Next →</Link>
              )}
            </div>
          </div>
        </>
